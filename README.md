...
- `scripts/rum.js` loads the Datadog Browser SDK with CORS enabled so the v7 Session Replay recorder chunk can resolve, and identifies Netlify, GitHub Pages, and local traffic with a `deployment` context value.
...
- The floating support control emits `support_cta_impression` once per page load and `support_cta_opened` when its popover opens. Support button clicks emit `developer_support_clicked` with the placement, selected tier, and amount, without payment or customer data. Their explicit `$5 coffee` and `$10 coffee and snack` action names remain visible while other automatic action names stay masked.
...
In browser developer tools, confirm that `datadog-rum.js` and its `datadogRecorder` chunk load, and that POST requests to both `/api/v2/rum` and `/api/v2/replay` return `202`. New applications can take a few minutes to appear in the [Datadog RUM Explorer](https://app.datadoghq.com/rum/performance-monitoring?query=%40application.id%3A709e7d93-9fec-4547-9721-154d38c929ae).
